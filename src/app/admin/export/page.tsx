...
/**
 * ✅ Admin Download Pro
 * - Filter: cabor, kategori, status pembayaran, status dokumen (uploaded/approved/rejected/empty)
 * - Export Excel:
 *   - Sheet "ALL" (rekap semua)
 *   - Sheet per CABOR (opsional)
 * - Export PDF:
 *   - Rekap tabel rapi + header + filter info
 *
 * Catatan:
 * - TIDAK pakai file-saver (biar tidak error typings). Download via Blob + <a>.
 * - Data bersumber dari localStorage `mg26_registration_<userId>` (mock FE).
 */
...
function getDocStatusForAthlete(reg: Registration, athleteId: string) {
  ...
  // ringkas "allStatus" untuk filter:
  // - jika ada REJECTED => REJECTED
  // - else jika ada UPLOADED => UPLOADED
  // - else jika semua APPROVED => APPROVED
  // - else EMPTY
  ...
  return (
    <div className="max-w-7xl space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900">
              Download Data Peserta (Pro)
            </h1>
            <p className="text-gray-600 mt-2">
              Export data peserta ke <b>Excel</b> dan <b>PDF</b> dengan filter lengkap. Output hanya memuat:
              <b> nama, kategori, asal instansi, no hp, official</b> (ditambah kolom admin seperti status).
            </p>
      ...
      <div className="bg-white border rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <div className="text-xs text-gray-500">
          Catatan: nomor HP atlet belum ada di form atlet, jadi export memakai nomor HP PIC (akun kontingen).
        </div>
      </div>
    </div>
  )
