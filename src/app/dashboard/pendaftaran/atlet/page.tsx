...
export default function Step3AtletPage() {
  ...
  const sportOptions = useMemo(() => {
    return (state.sports || []).filter((s: any) => {
      const q = Number(s.athleteQuota ?? s.plannedAthletes ?? 0)
  ...
  const sportQuota = useMemo(() => {
    ...
    if (!s) return 0
    return Number(s.athleteQuota ?? s.plannedAthletes ?? 0)
  }, [selectedSport])

  // ✅ filledInSport pakai "atlet lengkap" (profil + dokumen required)
  const filledInSport = useMemo(() => {
    if (!selectedSportId) return 0
    return (state.athletes || []).filter((a) => {
      if (a.sportId !== selectedSportId) return false
      const doc = (state.documents || []).find((d) => d.athleteId === a.id)
      return isAthleteComplete(a, doc as any)
    }).length
  }, [state.athletes, state.documents, selectedSportId])

  const remainingInSport = sportQuota - filledInSport

  // ==== Ringkasan global ====
  const totalPlan = useMemo(() => {
    return (state.sports || []).reduce((acc: number, s: any) => acc + Number(s.athleteQuota ?? s.plannedAthletes ?? 0), 0)
  }, [state.sports])
            ...
            {athleteForms.map((a, idx) => (
              <div key={idx} className="rounded-xl border p-4">
                ...
                <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <div className="text-sm font-bold mb-1">Nama Atlet</div>
                    <input
                      value={a.name}
                  ...
                  <div>
                    <div className="text-sm font-bold mb-1">Jenis Kelamin</div>
                    <select
                      value={a.gender}
                  ...
                  <div>
                    <div className="text-sm font-bold mb-1">Tanggal Lahir</div>
                    <input
                      type="date"
                      value={a.birthDate}
                  ...
                  <div>
                    <div className="text-sm font-bold mb-1">Asal Instansi Atlet (opsional)</div>
                    <input
                      value={a.institution}
