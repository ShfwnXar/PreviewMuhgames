...
export default function Step2PembayaranPage() {
  ...
  const onPickProof = async (file: File | null) => {
    ...
    // batas ukuran supaya aman (walau tidak simpan base64)
    const maxMB = 5
    ...
    // ✅ Simpan blob ke IndexedDB + simpan metadata (tanpa base64 di state)
    try {
      // pakai pola id yang kamu minta
      const fileId = `pay_${state?.payment ? "user" : "user"}_${Date.now()}_${Math.random().toString(16).slice(2)}`
      // ^ kalau kamu punya user.id di page ini, ganti bagian "user" jadi user.id (lihat catatan di bawah)

      await putFileBlob(fileId, file)

      setPaymentProof(fileId, file.name, file.type || "application/octet-stream")
            ...
            <div className="rounded-2xl border bg-white p-5">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="rounded-2xl border bg-gradient-to-br from-white to-emerald-50/60 p-4">
                  ...
                  <div className="mt-1 text-xs text-gray-500">Atlet 100k/orang, Official 50k/orang, Voli 1.2jt/tim.</div>
                </div>
